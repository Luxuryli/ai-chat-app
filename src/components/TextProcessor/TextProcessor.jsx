...
const TextProcessor = () => {
  ...
  const [setIsDropdownOpen] = useState(null);
  ...
  const handleSummarize = async (messageId) => {
    ...
    try {
      ...
      setErrorMessage("An error occurred while summarizing.");
        setTimeout(() => {
    setErrorMessage(""); // Clear the error after 3 seconds
  }, 3000);
    } finally {
...
const handleTranslate = async (messageId, targetLang) => {
  ...
  try {
    ...
    setErrorMessage(""); // ✅ Ensure error is cleared when translation succeeds
  } catch (error) {
    console.error("Translation error:", error);
     console.log("Setting error message:", `Translation to ${targetLang} failed. Please try again.`);

    // ✅ Directly update state to make sure UI reflects the error
      ...
      <div className={design.chat} ref={chatContainerRef}>
        {messages.map((msg) => (
          <div key={msg.id} className={design.messageContainer}>
            ...
            <div className={design.additonalInfoo}>
                {msg.summary && <p className={design.summarizer}><strong>Summary:</strong> {msg.summary}</p>}
            </div>
          </div>
        ))}
      </div>
      <div className={design.inputField}>
        <InputField value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Enter text here..." 
        />
